...
var getRequiredInput = (input) => (0, import_core.getInput)(input, { required: true }).trim();
...
var fotaFile = getRequiredInput("fota file");
var multilineOrUndefined = (s) => s.length > 0 ? s.split("\n") : void 0;
var abortOn = multilineOrUndefined((0, import_core.getInput)("abort on"));
var endOn = multilineOrUndefined((0, import_core.getInput)("end on"));
var endOnWaitSeconds = parseInt((0, import_core.getInput)("end on waitSeconds"), 10);
var certDir = getRequiredInput("certificate location");
var flashLogLocation = getRequiredInput("flashLog output");
var deviceLogLocation = getRequiredInput("deviceLog output");
var jobLocation = getRequiredInput("job output");
var testEnv = {
  ...
  brokerHostname: getRequiredInput("broker hostname"),
  stackName: getRequiredInput("stack name")
};
var powerCycle = getRequiredInput("powerCycle enabled") === "true" ? {
...
var tries = parseInt(getRequiredInput("tries"), 10);
...
var run = async () => {
  ...
  p.on("close", (code) => {
    clearTimeout(t);
    (0, import_core.setOutput)("connected", code === 0);
    if (timedOut) {
      console.error("Timed out.");
      (0, import_core.setOutput)("timeout", true);
      (0, import_core.setOutput)("try", numTry);
      process.exit(-108);
    }
    if (code === 0)
      process.exit();
